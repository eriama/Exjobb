...
void baseRateTask(void *arg)
{
  ...
  while (rtmGetErrorStatus(beaglebone_led_dimmer_M) == (NULL) ) {
    /* Wait for the next timer interrupt */
    MW_sigWait(&info.sigMask);
    beaglebone_led_dimmer_output();

    /* Get model outputs here */
    beaglebone_led_dimmer_update();
  }                                    /* while */
...
void MW_exitHandler(int sig)
{
  rtmSetErrorStatus(beaglebone_led_dimmer_M, "stopping the model");
  sem_post(&stopSem);
}
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(beaglebone_led_dimmer_M, 0);
  ...
  beaglebone_led_dimmer_initialize();
  ...
  if (rtmGetErrorStatus(beaglebone_led_dimmer_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(beaglebone_led_dimmer_M));
  }
  ...
  /* Terminate model */
  beaglebone_led_dimmer_terminate();
  return 0;
}
